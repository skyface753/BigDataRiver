...
class BdrResource:
  def on_get(self, req, resp):
    ...
    if 'product-kappa' in input:
      ...
      resp.body = json.dumps({"product":product, "recommendedProducts":result})
      ...
      resp.body = json.dumps({"product":product, "recommendedProducts":result, "from_stream":stream_rows[0].other_products if stream_rows else [], "from_batch":batch_rows[0].other_products if batch_rows else []})
    elif 'user' in input:
      user = int(input['user'])
      ...
      resp.body = json.dumps({"user":user, "recommendedProducts":result})
    else:
  ...
  def on_post(self, req, resp):
    msg = json.dumps(req.context['doc'])
...
class HealthCheckResource:
    def on_get(self, req, resp):
        resp.status = falcon.HTTP_200
        resp.body = json.dumps({"healthcheck":"ok"})
